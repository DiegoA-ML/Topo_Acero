...
# Use list comprehension to modify column names and assign them back to the DataFrame
clean_df_CNA.columns = [col.replace("_", " ").strip() for col in clean_df_CNA.columns]
clean_df_Prod.columns = [col.replace("_", " ").strip() for col in clean_df_Prod.columns]
clean_df_Impo.columns = [col.replace("_", " ").strip() for col in clean_df_Impo.columns]
clean_df_Expo.columns = [col.replace("_", " ").strip() for col in clean_df_Expo.columns]
...
"""# Graphs of products in CNA

**Seleccionan ciertos productos representativos para analizar el CNA **
"""

#Productos elegidos a analizar
nombre_productos = [
    #Materia prima
    "Mineral de Hierro",
    "Carbón Mineral",
    "Coque",
    "Chatarra",
    "Material Relaminable",#5 productos de materia prima
    #"Horno Eléctrico",#Dudoso
    #"Convertidor al Oxígeno",#Dudoso
    ...
    #Semiterminados
    "Planchón",
    "Lingotes, Palanquillas, etc.",

    ##Terminados
    #Planos
    "Placa",

    #Lamina con Recubrimiento
...
nombre_productos = [item+" CNA" for item in nombre_productos]
nombre_productos
...
for i in range(len(nombre_productos)):
  plt.figure(figsize = (10,6))
  plt.plot(indexes,clean_df_CNA[nombre_productos[i]])
  plt.title(nombre_productos[i])
  plt.xlabel('Meses')
  plt.ylabel('Monto')
  plt.show()

"""## Stastical Values: Mean, Median, Mode and"""

clean_df_CNA[nombre_productos].describe()

# Calculate the mode for each specified column
modes = clean_df_CNA[nombre_productos].mode().iloc[0,:]# Select the first mode if there are multiples.
modes
...
for x in range(len(nombre_productos)):
  plt.figure(figsize = (10,6))
  sns.boxplot(clean_df_CNA[nombre_productos[x]])
  plt.title(nombre_productos[x])
  plt.ylabel('Monto')
  plt.show()
...
for x in range(len(nombre_productos)):
  plt.figure(figsize = (10,6))
  plt.hist(clean_df_CNA[nombre_productos[x]])
  plt.title(nombre_productos[x])
  plt.xlabel('Monto')
...
clean_df_CNA[nombre_productos].corr()

import seaborn as sns
plt.figure(figsize=(20, 12))
sns.heatmap(clean_df_CNA[nombre_productos].corr(), cmap = 'coolwarm')

"""# Group CNA EDA

## Groups chosen for EDA
"""
...
#Heatmap and Correlation Matrix
 ...
 #Productos elegidos a analizar
grupo_productos = [ "TOTAL MATERIAS PRIMAS","BASICOS","SEMITERMINADOS","TERMINADOS"]
grupo_productos = [item+" CNA" for item in grupo_productos]
grupo_productos

"""## Time Series of Groups"""

for col in clean_df_CNA[grupo_productos]:
  plt.figure(figsize = (10,6))
  plt.plot(indexes,clean_df_CNA[col])
  plt.title(col)

"""## Statistical Values"""

clean_df_CNA[grupo_productos].describe()

clean_df_CNA[grupo_productos].mode().iloc[0,:]#Grab the first mode of each group

"""## Boxplots"""

for x in range(len(grupo_productos)):
  plt.figure(figsize = (10,6))
  sns.boxplot(clean_df_CNA[grupo_productos[x]])
  plt.title(grupo_productos[x])

"""## Histograms"""

for col in clean_df_CNA[grupo_productos]:
  plt.figure(figsize = (10,6))
  plt.hist(clean_df_CNA[col])
  plt.title(col)

#Los primeros 3 grupos de productos indican ser asimetricas claramente y la última puede ser considerada como simetrica.

"""## Heatmap and Correlation matrix"""

clean_df_CNA[grupo_productos].corr()

import seaborn as sns
sns.heatmap(clean_df_CNA[grupo_productos].corr(), cmap = 'coolwarm')
